...
|---|---|---|
| **Phase 0** | Accumulating Runtime Knowledge Graph (ARKG) — the foundational data layer | Closed: the graph, its ingestion pipeline and its MCP tools are in, and the `state_keys` and `git_commits` nodes that waited on Phases 1 and 3 landed with them |
| **Phase 1** | Runtime-to-Source Intelligence: source mapping with confidence scoring, causal threading, "Why did this render?" | Closed: source mapping, the locator, causal threading, state-store inspection and render blame are all in. Module-level Zustand stores are refused on measured evidence rather than pending |
| **Phase 2** | Autonomous bug reproduction, "Why is this value here?" provenance, Interaction-to-Test compiler, natural language app navigator | Closed: the compiler exports Playwright and Cypress, provenance and the navigator answer, and the replay harness runs the spec the export writes. Patch generation is refused — DevFlow diagnoses and verifies, and does not patch |
