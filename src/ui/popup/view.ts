 ...
 *
 * ## One surface, and why the second one went
 *
 * This popup used to locate as well as record, in a detached window over this
 * same document — a popup cannot do that job in the window it opens in, because
 * the click that reaches the page is the one that dismissed the popup. That
 * window is gone, and so is everything here that described it.
 *
 * It could show an answer but not act on one. `Open in Sources` needs a DevTools
 * window to reveal a compiled position in and a popup has none, so a located
 * component arrived at a card whose most useful action was missing — a dead end
 * at exactly the point the user wanted to move. Locating is the DevTools panel's
 * now, undivided: it has the Sources window, the parent tree and the history
 * drawer, and one place to look for a component beats two that disagree about
 * what they can do.
 */
