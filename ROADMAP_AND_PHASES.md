...
>
> **One open item is a gap rather than a decision, and is flagged as one** so that
> "closed as scoped" is not read as "nothing is left that could be built": the
> animated render graph in §3.3. Its data ships, its home is the viewer, and
> nobody has built it. Everything else unticked in Phases 0–3 is a refusal or a
> deferral with the argument written down.
>
...
- [~] **Interactive Graph Queries:** — **the data ships and one of the two questions is answered; the animated graph is a real gap, named as one below.**
  ...
  - [ ] *"Show me everything that renders when I click checkout" → animated real-time graph.* **Not built, and unlike everything else unticked in Phases 0–3 this is a gap rather than a refusal. It is buildable today and was not in the scope of the work that closed this phase.**

    The **data** ships twice over — 1.4's render blame says what re-rendered under a step, 1.3's causal graph says what a click caused — and the flow review already reads `FlowRenders` per step and prints it. So what is missing is one visualisation of facts a recording already carries, and its home is the **viewer's flow review**, which walks a recording step by step and has the data in hand. An earlier draft of this line argued against it as "a second component-tree view beside the panel's **Parent tree** and **Siblings**". That argument is about the wrong surface — the panel is the *locator*, and nothing proposed here belongs in it — and naming it here would make this refusal look better founded than it is. It is not a refusal. It is the one item in Phases 0–3 that somebody could pick up tomorrow.

    The two things worth settling before anyone does: whether an animation earns itself over the step-ordered list that already exists, and that "real-time" means *replaying a finished recording* here, not watching a live page — §3.3 above having established that this product does not hold a live feed.
- [x] **MCP Tool:**
